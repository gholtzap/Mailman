...
def generate_text_from_paper(pdf_url):
    print("Process Start")

    print("Loading read_paper_prompt")
    prompt_template = load_read_paper_prompt()
    ...
    is_valid, page_count = check_pdf_page_count(pdf_content)

    if not is_valid:
        print("\nSKIPPED: Paper exceeds 25-page limit")
        print("   Use --dry-run to preview papers before processing")
        print("   Most quality research papers are under 25 pages")
        sys.exit(2)  # Exit code 2 = skipped (not an error)
...
def humanize_text(text):
    ...
    humanizer_prompt_template = load_humanizer_prompt()
